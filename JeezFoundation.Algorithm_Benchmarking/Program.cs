...
using static JeezFoundation.Algorithm.CommandLine;
		...
		string? includeRegex = null)
	{
		...
		if (updateDocumentation)
		{
			...
			string[] lines =
			{
				...
				"",
				"The source code for all becnhmarks are in [Tools/ JeezFoundation.Algorithm.Benchmarking](https://github.com/ZacharyPatten/Towel/tree/main/Tools/Towel_Benchmarking).",
				"",
			};
		...
		foreach (Type type in Benchmarks)
		{
			if (includeRegex is null || Regex.Match(type.Name, includeRegex).Success)
			{
				...
				stringBuilder.AppendLine();
				stringBuilder.AppendLine("The source code for all benchmarks are in [Tools/JeezFoundation.Algorithm.Benchmarking](https://github.com/ZacharyPatten/Towel/tree/main/Tools/Towel_Benchmarking).");
				stringBuilder.AppendLine();
